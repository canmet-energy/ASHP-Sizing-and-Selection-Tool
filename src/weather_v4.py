...
def run_all_scenarios(folder_location: str = 'energy_plus_weather', 
                     num_processes: Optional[int] = None) -> None:
    """
    ...
    1. Download all Canadian weather files (if not already present)
    2. Run all 6 degree hour scenarios:
       - hdh_sc1.csv: Heating scenario 1 (standard)
       - hdh_sc2.csv: Heating scenario 2 (conservative)  
       - hdh_sc3.csv: Heating scenario 3 (advanced)
       - cdh_sc1.csv: Cooling scenario 1 (standard)
       - cdh_sc2.csv: Cooling scenario 2 (conservative)
       - cdh_sc3.csv: Cooling scenario 3 (advanced)
    ...
    logger.info("Step 1: Downloading Canadian weather files...")
    download_weather_files(folder_location)
    
    # STEP 2: PROCESS ALL SCENARIOS
    logger.info("Step 2: Running all scenarios with parallel processing...")
    
    total_start = time.time()
    # Run each of the 6 predefined scenarios
    for scenario_name in PREDEFINED_SCENARIOS.keys():
        run_scenario(scenario_name, folder_location, num_processes)
...
if __name__ == "__main__":
    ...
    python weather_v4.py                           # Use default folder (output)
    python weather_v4.py --folder my_results       # Save to custom folder 
    python weather_v4.py --benchmark               # Run in benchmark mode
    python weather_v4.py --folder my_output --benchmark  # Custom folder + benchmark
    """
    import argparse
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='''
Examples:
  python weather_v4.py                              # Default settings (saves to 'output' folder)
  python weather_v4.py --folder my_results          # Custom output folder
  python weather_v4.py --benchmark                  # Performance testing mode
  python weather_v4.py --folder my_output --benchmark  # Both custom folder and benchmark
        '''
    )
    ...
    parser.add_argument(
        '--folder', 
        default='output',
        help='Output folder for weather files and results (default: output)'
    )
    ...
    if args.benchmark:
        logger.info(f"Running in benchmark mode with output folder: {args.folder}")
        start_time = time.time()
        run_all_scenarios(folder_location=args.folder, num_processes=cpu_count())
        total_time = time.time() - start_time
        logger.info(f"🚀 Total execution time: {total_time:.2f}s")
    else:
        logger.info(f"Running with output folder: {args.folder}")
        run_all_scenarios(folder_location=args.folder)